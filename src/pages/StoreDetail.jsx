    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-400 text-sm font-medium">Chargement de la boutique...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center p-4">
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 max-w-md text-center">
          ...
          <h2 className="text-xl font-bold text-white mb-2">Boutique introuvable ou inactive</h2>
          <p className="text-sm text-slate-400 mb-6">Cette boutique n'existe plus ou a été désactivée par l'administration.</p>
          <Link
            to="/"
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div className="flex items-start sm:items-center gap-5">
              <img
                src={vendor.logo}
                alt={vendor.name}
                className="w-20 h-20 sm:w-24 sm:h-24 rounded-3xl object-cover border-2 border-emerald-500/40 shadow-xl shrink-0"
              />
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <h1 className="text-2xl sm:text-3xl font-black text-white">{vendor.name}</h1>
                  <span className="px-2.5 py-0.5 rounded-full bg-emerald-950 border border-emerald-500/40 text-emerald-400 text-xs font-bold flex items-center gap-1">
                    <ShieldCheck className="w-3.5 h-3.5" /> Grossiste Vériﬁé
                  </span>
                </div>
                ...
                {vendor.bio && (
                  <p className="text-xs text-slate-300 mt-2 max-w-xl leading-relaxed">{vendor.bio}</p>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 shrink-0">
              {cleanWhatsapp && (
                <a
                  ...
                  className="px-4 py-3 rounded-2xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs sm:text-sm shadow-lg shadow-emerald-600/30 transition-all flex items-center gap-2"
                >
              ...
              {vendor.phone && (
                <a
                  ...
                  className="px-4 py-3 rounded-2xl bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 font-bold text-xs sm:text-sm transition-all flex items-center gap-2"
                >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-xs text-slate-400">Prix de gros HT réservés aux professionnels de parapharmacie</p>
          </div>
          ...
          <div className="relative w-full sm:w-72">
            ...
            <input
              ...
              className="w-full bg-slate-900 border border-slate-800 text-white placeholder-slate-500 pl-9 pr-3 py-2 rounded-xl text-xs font-medium focus:outline-none focus:border-emerald-500"
            />
          </div>
        </div>
        ...
        {filteredProducts.length > 0 ? (
          ...
          <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-12 text-center max-w-md mx-auto">
            ...
            <h3 className="text-lg font-bold text-white mb-1">Aucun produit dans cette boutique</h3>
            <p className="text-sm text-slate-400">Ce grossiste n'a pas encore ajouté de produit correspondant.</p>
          </div>
        )}
      </main>
